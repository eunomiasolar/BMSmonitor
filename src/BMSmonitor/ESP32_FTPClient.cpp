...
//extern "C" {
extern int ReadFileUpCall(unsigned char * Buffer, int Len);
//}
...
//void ESP32_FTPClient::WriteClientUpCall(int (*ReadCall(unsigned char *buf, int len)) )
void ESP32_FTPClient::WriteClientUpCall(void)
{
  size_t count = 0;
  WiFiClient* cli;
  ...
  while(1)
  {
    //count = ReadCall(&clientBuf[0], 512);
    count = ReadFileUpCall(&clientBuf[0], 512);
    if(count < 512) 
    {
      if(count > 0) 
        cli->write(clientBuf, count);
      break; 
    }
    else
    {
      cli->write(clientBuf, 512); 
    }
  }
}
